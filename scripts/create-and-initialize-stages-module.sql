...
-- À exécuter avec la commande : PGPASSWORD="Apollonf@vi92" psql -h db.epnhnjkbxgciojevrwfq.supabase.co -p 5432 -U postgres -d postgres -f scripts/create-and-initialize-stages-module.sql
...
CREATE TABLE IF NOT EXISTS stage_offres (
    ...
    duree INTEGER,
    professeur_id UUID REFERENCES professors(id),
...
CREATE TABLE IF NOT EXISTS stage_candidatures (
    ...
    etudiant_id UUID REFERENCES students(id),
...
BEGIN
  WITH student_schedule AS (
    ...
    JOIN professors p ON s.professor_id::uuid = p.id
    WHERE s.student_id = student_id::text
  ),
  student_news AS (
    SELECT 
      n.id,
