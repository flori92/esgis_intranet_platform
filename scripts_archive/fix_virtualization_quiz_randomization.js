/**
 * Script pour améliorer le quiz de virtualisation avec 40 questions et randomisation
 * Ce script ajoute plus de questions au quiz et implémente la randomisation
 */
...
// Fonction principale
const main = () => {
  console.log("=== AMÉLIORATION DU QUIZ DE VIRTUALISATION ===");
  
  try {
    // Vérifier si le fichier de données du quiz existe
    ...
    // Lire le contenu du fichier de données du quiz
    let quizDataContent = fs.readFileSync(quizDataPath, 'utf8');
    
    // Créer 30 questions supplémentaires pour atteindre un total de 40
    ...
    // Ajouter les questions supplémentaires au fichier
    ...
    // Ajouter la fonction de randomisation des questions
...
`;
      ...
      quizDataContent = quizDataContent.replace(
        /export const virtualizationQuizData = {[\s\S]*?};/,
        match => match + randomizationFunction
      );
    }
    
    // Écrire le contenu modifié dans le fichier
    fs.writeFileSync(quizDataPath, quizDataContent, 'utf8');
    console.log(`Quiz de virtualisation amélioré avec 40 questions dans ${quizDataPath}`);
    
    // Modifier le composant QuizLauncher pour utiliser la randomisation
    ...
    if (fs.existsSync(quizLauncherPath)) {
      ...
      if (!quizLauncherContent.includes('getRandomizedQuestions')) {
        ...
        // Modifier la logique pour utiliser la fonction de randomisation
        quizLauncherContent = quizLauncherContent.replace(
          /setQuizData\(examId\.includes\('virtualization'\) \? virtualizationQuizData : sampleQuizData\);/,
          `if (examId.includes('virtualization')) {
        ...
        console.log(`Randomisation des questions ajoutée dans ${quizLauncherPath}`);
      } else {
    ...
    console.log("Amélioration du quiz de virtualisation terminée avec succès.");
  } catch (error) {
    console.error(`Erreur lors de l'amélioration du quiz de virtualisation:`, error);
  }
  
  console.log("=== FIN DE L'AMÉLIORATION DU QUIZ DE VIRTUALISATION ===");
};
