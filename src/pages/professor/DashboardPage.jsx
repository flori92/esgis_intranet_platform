...
const HeroBanner = ({ banners }) => {
  ...
  if (items.length === 0) {
    return (
      <Paper elevation={0} sx={{ position: 'relative', mb: 4, borderRadius: DASHBOARD_CARD_RADIUS, overflow: 'hidden', minHeight: { xs: 200, md: 300 }, background: `linear-gradient(135deg, ${NAVY} 0%, #0a4d8c 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <Paper elevation={0} sx={{ position: 'relative', mb: 4, borderRadius: DASHBOARD_CARD_RADIUS, overflow: 'hidden', minHeight: { xs: 220, md: 340 }, display: 'flex', alignItems: 'center', '&:hover .hero-nav': { opacity: 1 } }}>
      {items.map((b, i) => (
      ...
      <Box sx={{ position: 'relative', zIndex: 2, p: { xs: 3, md: 6 }, maxWidth: 700 }}>
        ...
        {banner.cta_text && banner.cta_link && <Button variant="contained" component={Link} to={banner.cta_link} sx={{ borderRadius: DASHBOARD_CARD_RADIUS, px: 4, py: 1.2, fontWeight: 700, bgcolor: RED, color: 'white' }}>{banner.cta_text}</Button>}
      </Box>
    </Paper>
  );
};
...
const ProfessorDashboardPage = () => {
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, maxWidth: 1440, mx: 'auto', animation: `${fadeIn} 0.5s ease-out` }}>
      ...
      {/* Insights Section */}
      {insights.summary && (
        <Paper elevation={0} sx={{ ...getDashboardPanelSx(NAVY), p: 3, mb: 4 }}>
          ...
          <Grid container spacing={2}>
            {[
              ...
              <Grid item xs={4} key={item.label}>
                <Box sx={{ ...getDashboardInsetSx(item.color), p: 2, textAlign: 'center' }}>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={7}>
          <Stack spacing={3}>
            <Paper elevation={0} sx={{ ...getDashboardPanelSx(NAVY), p: 3 }}>
              ...
              <Grid container spacing={2}>
                {(newsRes?.data || []).slice(0, 2).map((item, idx) => (
                  <Grid item xs={12} sm={6} key={item.id}>
                    <Card elevation={0} sx={{ ...getDashboardInteractiveCardSx(NAVY), height: '100%' }}>
          ...
          <Paper elevation={0} sx={{ ...getDashboardPanelSx(NAVY), p: 3, height: '100%' }}>
            ...
            <Stack spacing={1.5}>
              {(eventsRes?.data || []).slice(0, 4).map(event => {
                ...
                return (
                  <Paper key={event.id} elevation={0} sx={{ ...getDashboardInteractiveCardSx(NAVY), display: 'flex', overflow: 'hidden' }}>
                    <Box sx={{ width: 60, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', bgcolor: alpha(NAVY, 0.05), borderRight: `1px solid ${alpha(NAVY, 0.14)}` }}>
