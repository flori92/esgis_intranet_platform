-- Transform Materialized Views into Dynamic Standard Views for instant data refresh
-- ESGIS Campus § Performance & Real-time Analytics

-- 1. DROP Dependent Views and Materialized Views
DROP VIEW IF EXISTS public.v_student_global_attendance_stats CASCADE;
DROP MATERIALIZED VIEW IF EXISTS public.mv_professor_course_stats CASCADE;
DROP MATERIALIZED VIEW IF EXISTS public.mv_student_attendance_stats CASCADE;

-- 2. CREATE Dynamic View for Student Attendance (Always up to date)
...
-- 3. CREATE Global Stats View (Aggregated from the dynamic view)
...
-- 4. CREATE Dynamic View for Professor Stats (Always up to date)
...
-- 5. Cleanup assignments for Floriace FAVI
-- Keep ONLY course 9 (Virtualisation)
DELETE FROM public.professor_courses 
WHERE professor_id = '06b1308c-7b62-4e94-af1c-38d963ec8bb0' 
  AND course_id != 9;
